...
def sidebar_credits():
    ...
    st.sidebar.page_link(
        "https://joaoribeiromedeiros.github.io/", label="An app by João Ribeiro Medeiros! "
    )

    st.sidebar.page_link(
        "https://www.lesswrong.com/posts/rLQGD63B2znREaaP7/retrieval-augmented-genesis", label="Read the article!"
    )
...
if st.session_state.page != "Bibliography":
    ...
    if "encoder_model" not in st.session_state:
        ...
        st.session_state.encoder_model = st.sidebar.selectbox(
            "Select Embedding Model:", encoder_models, index=encoder_models.index(st.session_state.encoder_model)
        )
...
if st.session_state.page != "VerseUniVerse":
    ...
    if "selected_text" not in st.session_state:
        ...
        st.session_state.selected_text = st.sidebar.selectbox(
            "Select Holy Text",
            holy_texts,
            index=holy_texts.index(st.session_state.selected_text),
        )
...
if st.session_state.page == "VerseUniVerse":
    ...
    if "selected_texts" not in st.session_state:
        ...
        st.session_state.selected_texts = st.sidebar.multiselect(
            "Select Holy Texts", holy_texts, default=st.session_state.selected_texts
        )



def display_retrieval(results_sources, results_references, results_verses, target_variable='query'):
...
def verse_uni_verse():
    ...
    st.markdown(
        "Describe a subject or an idea you are interested in. Start a conversation! "
    )
    ...
    oracle = st.sidebar.radio("Toggle GenAI", ("On", "Off"), index=1, key="oracle")
    ...
    **Enjoy your journey!**            
    ...
    if st.session_state.oracle == "On":
        ...
        with st.spinner('Patience grasshopper...'):
            ...
            response = get_oracle_response(st.session_state.query + retrieval, local=local, agent = 'oracle')
...
def semantic_network(local=local):
    ...
    if "parameter" not in st.session_state:
        ...
        st.session_state.parameter = st.sidebar.selectbox(
            "Select Similarity threshold:", parameter_choices, index=parameter_choices.index(st.session_state.parameter),
        )
    ...
    **Enjoy your journey!**            
    ...
    if network_view == "Main Verses":
        ...
        semantic_network_stats = load_semantic_network_stats(
            [st.session_state.selected_text], encoder_model=st.session_state.encoder_model, threshold=float(st.session_state.parameter)
        )
        # for text in selected_texts:
        main_verses, main_references = write_main_verses_from_dict(
            st.session_state.selected_text, semantic_network_stats[st.session_state.selected_text]
        )
        ...
        if st.session_state.oracle == "On":
            ...
            with st.spinner('Patience grasshopper...'):
                ...
                retrieval = join_central_retrieved_references(main_references, main_verses)
                response = get_oracle_response(
                    retrieval, agent="exegete", local=local
                )
                st.markdown(response)
        ...
        plot_main_network_metrics_from_dict(
            ...
            local,
            file_path="data/analytics_data/" + st.session_state.encoder_model + "/" + parameter_path_dict[st.session_state.parameter] + "/"
        )

    elif network_view == "All Verses":

        if st.session_state.encoder_model+ st.session_state.parameter +st.session_state.selected_text not in st.session_state.g_rust:
            G_rust, target_verses = load_saved_rust_network( selected_text=st.session_state.selected_text, encoder_model=st.session_state.encoder_model, parameter = parameter_path_dict[st.session_state.parameter], local=local)
            st.session_state.g_rust[st.session_state.encoder_model+st.session_state.parameter+st.session_state.selected_text] = G_rust
            st.session_state.target_verses[st.session_state.selected_text] = (
                target_verses
            )

        elif st.session_state.encoder_model+ st.session_state.parameter +st.session_state.selected_text  in st.session_state.g_rust:
            G_rust = st.session_state.g_rust[st.session_state.encoder_model+st.session_state.parameter+st.session_state.selected_text]
...
def target_semantic_network(G_rust, target_verses):
    ...
    with st.spinner('Patience grasshopper...'):
        ...
        high_centrality_references = reorder_list(target_reference, high_centrality_references)
        ...
        display_retrieval(
            high_centrality_sources, high_centrality_references, high_centrality_verses, target_variable='verse'
        )
